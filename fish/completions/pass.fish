...
# OTP completions
function __fish_pass_print_otp_entries
    # 查找所有以 "otp." 开头的 .gpg 文件，并去掉前缀和扩展名
    set -l prefix (__fish_pass_get_prefix)
    set -l matches $prefix/**otp.*.gpg
    printf '%s\n' $matches | sed "s#$prefix/\(.*\)\.gpg#\1#"
end
...
complete -c $PROG -f -n '__fish_pass_uses_command otp' -s q -l qrcode -d 'Display QR code'
complete -c $PROG -f -n '__fish_pass_uses_command otp' -a "(__fish_pass_print_otp_entries)"
